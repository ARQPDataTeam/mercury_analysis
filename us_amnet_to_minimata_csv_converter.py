...
mercury_sites_df=pd.read_csv(sites_filename, engine='python', index_col=0)
timezone_dictionary={'Pacific/Honolulu':'UTC-10:00','America/Anchorage':'UTC-9:00','America/Los_Angeles':'UTC-8:00','America/Denver':'UTC-7:00','America/Mexico_City':'UTC-6:00','America/Chicago':'UTC-6:00','America/Indiana/Indianapolis':'UTC-5:00','America/New_York':'UTC-5:00','America/Detroit':'UTC-5:00'}

# # obj=TimezoneFinder() 
# for site in mercury_sites_df.index:
#     mercury_sites_df.loc[site,'utc_offset']=timezone_dictionary[mercury_sites_df.loc[site,'timezone']]

# mercury_sites_df.to_csv(sites_filename, index=True)

file_list=glob.glob('C:/Users/firanskib/OneDrive - EC-EC/mercury/US_AMNET_Files/minamata_format/*.csv')

for filename in file_list:    
    basename=os.path.basename(filename)
    print (basename)

    header_df=pd.read_csv(filename, index_col=0)
    print (header_df.loc['*SITE IDENTIFICATION',1])
    print(mercury_sites_df.loc[mercury_sites_df.loc[:,'siteName']==header_df.loc['*SITE IDENTIFICATION',1],'utc_offset'])
    # header_df.loc['*TIME ZONE',1]=mercury_sites_df.loc[mercury_sites_df.loc[:,'siteName']==header_df.loc['*SITE IDENTIFICATION',1],'utc_offset']
